...
export default  class DetalleRetiro{
    ...
    @Column({ default: true})
    @ApiProperty({
      description: 'Muestra si el detalle retiro se encuetra activo (sirve para el SoftDelete)',
    })
    is_active: boolean;

    @CreateDateColumn({ name: 'created_at', type: 'timestamp with time zone' })
    @ApiProperty({
      description: 'Fecha de Creaciòn del detalle retiro se inserta automaticamente',
    })
    createdAt: Date;
    ...
    @ApiProperty({
      description:
        'Ultima fecha de modificaciòn del detalle retiro',
    })
    updatedAt: Date;
